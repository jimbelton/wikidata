...
class TestWkExtract(unittest.TestCase):
    def testSimplify(self):
        ...
        linesNew  = subprocess.check_output([wdExtractPy, "-f", "-F", "-l", "en", "-s", "", inputFile]).split()
        diff      = list(difflib.context_diff(linesOut, linesNew))
        self.assertEqual(len(diff), 0, "Unexpected differences in output of wd-extract.py -f -F -l en -s '' data/1000.json\n"
                                       + "".join(diff))
